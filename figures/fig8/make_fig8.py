# make_fig8.py
# CLI wrapper: takes FOUR JSON inputs and produces Appendix Fig.8
...
from fig8_data import fig8AnalysisConfig, compute_fig8_products, build_fig8_manifest
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--tag", type=str, default="8", help="Tag used in output filenames (default: 8)")
...
def main() -> None:
    ...
    cfg = fig8AnalysisConfig()

    products = compute_fig8_products(
    ...
    ctx_csv = cache_dir / f"{args.tag}_derived_context_kappa.csv"
    sum_csv = cache_dir / f"{args.tag}_summary_metrics.csv"
    ctx_csv_root = out_dir / f"{args.tag}_derived_context_kappa.csv"
    sum_csv_root = out_dir / f"{args.tag}_summary_metrics.csv"
    ...
    # Render figure
    out_pdf = out_dir / f"Fig_7_{args.tag}.pdf"
    out_png = None if args.no_png else str(out_dir / f"Fig_7_{args.tag}.png")
    ...
    manifest_path = out_dir / f"figure_7_{args.tag}_manifest.json"
